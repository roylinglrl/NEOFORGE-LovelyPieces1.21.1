...
public class HKMP7Item extends GunItem {
    public HKMP7Item(Properties properties) {
        ...
        if (!level.isClientSide) {
            ...
            if (!hasBullet) {
                for (int i = 9; i < player.getInventory().getContainerSize(); i++) {
                    ItemStack stack = player.getInventory().getItem(i);
                    if (stack.getItem() instanceof FlintlockBulletItem) {
                        if(player.getRandom().nextDouble()> (double) 0.5) {
                            stack.shrink(1);
                        }
                        hasBullet = true;
                        break;
            ...
            //gunStack.set(ModDataComponents.SHOOT_COOLDOWN.get(),new ShootCDData(0));
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
        ...
        tooltipComponents.add(Component.translatable("tooltip.lovely_sparkle_pieces.hkmp7.inf").withColor(ColorUtil.getRainbow(2f)));
        tooltipComponents.add(Component.translatable("tooltip.lovely_sparkle_pieces.hkmp7.inf1").withColor(ColorUtil.getRainbow(2f)));
        ...
        tooltipComponents.add(Component.translatable("tooltip.lovely_sparkle_pieces.hkmp7.des1").withColor(ColorUtil.getRainbow(2f)));
